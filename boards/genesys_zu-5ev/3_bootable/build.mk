...
$(O_FSBL_ELF) $(O_APP_ELF)&: $(O_PLATFORM_XSA) $(O_LIBRUST_FIRMWARE_A) $(I_VITIS_SCRIPT_PY) \
 $(I_SYSTEM_USER_DTSI) $(I_XFSBL_DDR_INIT_C) $(I_BOOTABLE_SRC_wildcard_CHS)
	env -C "$(O)" -- vitis -s "$(I_VITIS_SCRIPT_PY)" -W "$(O_VITIS_WS)" -P "$(O_PLATFORM_XSA)" \
	  -D "$(I_SYSTEM_USER_DTSI)" -S "$(I_BOOTABLE_SRC)" -R "$(O_RUST_FW_DIR)"
